...
public class CoordinateFormatter {
    ...
    public String sexagesimal(double value, int precision) {
        ...
        if (precision <= 2) {
            ...
            fmin = 60. * (value - deg);
            min = (int) fmin;
            double fsec = 60. * (fmin - min);
            sec = (int) fsec;
            double ffrac = fsec - sec;
    ...
    public String decimal(double value, int precision) {
        ...
        StringBuilder form = new StringBuilder("0");
        if (zeroFill) {
            form.append("0");
        }
        form.append(".");
        for (int i = 2; i < precision; i += 1) {
            form.append(zeroFill ? "0" : "#");
        }

        return new DecimalFormat(form.toString()).format(value);
    }
    ...
    private String numb(int input) {
        return !zeroFill || input >= 10 ? "" + input : "0" + input;
    }
